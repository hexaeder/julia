# switch from `IRCode` interpreter to normal inference interpreter
# where irinterp can recurse back to the normal inference

function typeinf_edge(irinterp::AbstractInterpreter, method::Method, @nospecialize(atype), sparams::SimpleVector, caller::IRInterpretationState)
    interp = switch_from_irinterp(irinterp)
    return @invoke typeinf_edge(interp::AbstractInterpreter, method::Method, atype::Any, sparams::SimpleVector, caller::AbsIntState)
end

function abstract_call_method_with_const_args(irinterp::AbstractInterpreter,
    result::MethodCallResult, @nospecialize(f), arginfo::ArgInfo, si::StmtInfo, match::MethodMatch,
    sv::IRInterpretationState, invokecall::Union{Nothing,InvokeCall})
    interp = switch_from_irinterp(irinterp)
    return @invoke abstract_call_method_with_const_args(interp::AbstractInterpreter,
        result::MethodCallResult, f::Any, arginfo::ArgInfo, si::StmtInfo, match::MethodMatch,
        sv::AbsIntState, invokecall::Union{Nothing,InvokeCall})
end
