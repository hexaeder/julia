...
let code = code_lowered(FieldConvert)[1].code
    ...
    @test code[7] == Expr(:(=), Core.SlotNumber(7), Expr(:call, GlobalRef(Base, :convert), Core.SSAValue(2), Core.SlotNumber(2)))
    @test code[8] == Core.SlotNumber(7)
    @test code[9] == Expr(:call, GlobalRef(Core, :fieldtype), Core.SSAValue(1), 2)
    @test code[14] == Expr(:(=), Core.SlotNumber(8), Expr(:call, GlobalRef(Base, :convert), Core.SSAValue(9), Core.SlotNumber(3)))
    @test code[15] == Core.SlotNumber(8)
    @test code[16] == Expr(:call, GlobalRef(Core, :fieldtype), Core.SSAValue(1), 4)
    @test code[21] == Expr(:(=), Core.SlotNumber(9), Expr(:call, GlobalRef(Base, :convert), Core.SSAValue(16), Core.SlotNumber(5)))
    @test code[22] == Core.SlotNumber(9)
    @test code[23] == Expr(:call, GlobalRef(Core, :fieldtype), Core.SSAValue(1), 5)
    @test code[28] == Expr(:(=), Core.SlotNumber(10), Expr(:call, GlobalRef(Base, :convert), Core.SSAValue(23), Core.SlotNumber(6)))
    @test code[29] == Core.SlotNumber(10)
